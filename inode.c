                                         ...
                                         struct buffer_head **p)
{
	...
	*p = NULL;
	...
	offset = ((ino - 1) % inodes_pg) * inode_sz; // h thesh toy inode ston pinaka * to megethos toy inode
	/* Return the pointer to the appropriate ext2_inode */
	block = le32_to_cpu(gdp->bg_inode_table) + (offset / blocksize);
	bh = sb_bread(sb, block);
	*p = bh;
	offset = offset % blocksize; // to offset mesa sto block poy phrame apo to sb_bread
